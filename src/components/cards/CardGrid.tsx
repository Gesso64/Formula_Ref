...
export function CardGrid() {
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, padding: '1.5rem 1.2rem 6rem', maxWidth: mode === 'examples' ? 760 : 1200, margin: '0 auto', width: '100%' }}>
      {/* Header */}
      <div style={{ marginBottom: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 2 }}>
          <h1 style={{ fontSize: 21, fontWeight: 500, margin: 0 }}>
            {mode === 'formulas' ? 'Formula & Concept Reference' : 'Worked Examples'}
          </h1>
          {hasExamples && (
            <div style={{ display: 'flex', gap: 3, background: 'var(--color-bg2)', borderRadius: 8, padding: 3, border: '0.5px solid var(--color-border)' }}>
              {(['formulas', 'examples'] as Mode[]).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  style={{
                    fontSize: 11.5, padding: '4px 12px', borderRadius: 6, border: 'none', cursor: 'pointer', fontFamily: 'inherit',
                    background: mode === m ? 'var(--color-surface)' : 'transparent',
                    color: mode === m ? 'var(--color-text)' : 'var(--color-text3)',
                    fontWeight: mode === m ? 500 : 400,
                    boxShadow: mode === m ? '0 0.5px 2px rgba(0,0,0,.08)' : 'none',
                    transition: 'all .15s',
                  }}>
                  {m === 'formulas' ? 'Formulas' : 'Worked Examples'}
                </button>
              ))}
            </div>
          )}
